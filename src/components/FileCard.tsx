...
const FileCard: React.FC<FileCardProps> = ({ layout, onActionClick, doc }) => {
    ...
    return (
        <div
            className={`relative group bg-white dark:bg-gray-700 rounded-xl p-4 w-full shadow hover:shadow-md cursor-pointer 
            ${layout === "list" ? "flex items-center gap-3 max-w-full" : "max-w-[200px] text-center"}`}
        >
            {/* Nút ba chấm hiển thị khi hover */}
            <button
                onClick={handleMenuClick}
                className="absolute top-4 right-2 p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-600 invisible group-hover:visible"
            >
